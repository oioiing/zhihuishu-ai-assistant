...
(function() {
    ...
    function displayHomeworkAnalysis(analysisData, croppedImage, extractedText = '') {
        const isObject = typeof analysisData === 'object';
        const score = isObject ? analysisData.score : (analysisData.match(/总分：(\d+)/)?.[1] || '');
        const feedback = isObject ? analysisData.feedback : analysisData;
        const details = isObject ? analysisData.details : '';
        const improvement = isObject ? analysisData.improvement : '';
        ...
        if (croppedImage) {
            ...
            contentHtml += `
                <div style="margin-bottom:16px; padding:12px; background:#f8f9fa; border-radius:8px; font-size: 12px; color: #666; max-height: 100px; overflow-y: auto;">
                    <div style="font-weight:600; margin-bottom:4px;">📝 提取文本</div>
        ...
        const html = `
            <div style="max-height:500px; overflow-y:auto; padding:10px;">
                ...
                <div style="background:#fff; border-radius:8px; border:1px solid #e5e7eb;">
                    <div style="background:#FF6B6B; color:white; padding:12px; border-radius:8px 8px 0 0; font-weight:600; display: flex; justify-content: space-between;">
                        <span>🤖 AI智能批改结果</span>
                        ${score ? `<span style="background: rgba(255,255,255,0.2); padding: 0 8px; border-radius: 4px;">得分: ${score}</span>` : ''}
                    </div>
                    <div style="padding:16px; line-height:1.6; font-size:14px;">
                        <div style="font-weight: 600; color: #333; margin-bottom: 4px;">总体评价:</div>
                        <div style="white-space: pre-wrap; margin-bottom: 12px;">${feedback}</div>

                        ${details ? `
                            <div style="font-weight: 600; color: #333; margin-bottom: 4px;">批改细节:</div>
                            <div style="white-space: pre-wrap; margin-bottom: 12px; font-size: 13px;">${details}</div>
                        ` : ''}

                        ${improvement ? `
                            <div style="font-weight: 600; color: #333; margin-bottom: 4px;">改进建议:</div>
                            <div style="white-space: pre-wrap; font-size: 13px;">${improvement}</div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div style="margin-top:16px; text-align:center; display:flex; gap:8px; justify-content:center; flex-wrap: wrap;">
                    <button id="zh-auto-fill-btn" style="background:#4FC3F7; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer; font-size:12px; font-weight: 600;">
                        ✍️ 自动填分
                    </button>
                    <button id="zh-screenshot-again-btn" style="background:#4CAF50; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer; font-size:12px;">
                        📸 重新截图
                    </button>
                    <button id="zh-save-analysis-btn" style="background:#2196F3; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer; font-size:12px;">
                        💾 保存批改
                    </button>
                </div>
            </div>
        ...
        // 绑定事件 (确保元素存在后再操作)
        const panelBody = document.getElementById('zh-panel-body');
        if (panelBody) {
            const autoFillBtn = panelBody.querySelector('#zh-auto-fill-btn');
            if (autoFillBtn) {
                autoFillBtn.onclick = (e) => {
                    e.stopPropagation();
                    autoFillGradeAndComment(score, feedback + (details ? '\n\n细节：' + details : '') + (improvement ? '\n\n改进建议：' + improvement : ''));
                };
            }

            const screenshotAgainBtn = panelBody.querySelector('#zh-screenshot-again-btn');
            if (screenshotAgainBtn) {
                screenshotAgainBtn.onclick = (e) => {
                    e.stopPropagation();
                    activateScreenshotMode();
                };
            }

            const saveAnalysisBtn = panelBody.querySelector('#zh-save-analysis-btn');
            if (saveAnalysisBtn) {
                saveAnalysisBtn.onclick = (e) => {
                    e.stopPropagation();
                    const fullText = `智慧树AI助教 - 作业批改报告\n生成时间: ${new Date().toLocaleString()}\n\n得分: ${score}\n\n评价: ${feedback}\n\n细节: ${details}\n\n建议: ${improvement}`;
    ...
    function autoFillGradeAndComment(score, comment) {
        console.log('✍️ 开始自动填充成绩和评语...', { score, commentLength: comment.length });

        try {
            // 查找分数输入框
            const scoreInputs = [
                ...document.querySelectorAll('input[placeholder*="成绩"]'),
                ...document.querySelectorAll('input.el-input__inner[type="number"]'),
                ...document.querySelectorAll('.score-input input'),
                ...document.querySelectorAll('input[aria-label*="成绩"]')
            ].filter(el => el.offsetParent !== null); // 只取可见的

            if (scoreInputs.length > 0 && score) {
                ...
                console.warn('⚠️ 未找到可见的分数输入框');
            }

            // 查找评语输入框
            const commentInputs = [
                ...document.querySelectorAll('textarea[placeholder*="评语"]'),
                ...document.querySelectorAll('textarea.el-textarea__inner'),
                ...document.querySelectorAll('.comment-textarea textarea'),
                ...document.querySelectorAll('textarea[aria-label*="评语"]')
            ].filter(el => el.offsetParent !== null);

            if (commentInputs.length > 0 && comment) {
                ...
                console.warn('⚠️ 未找到可见的评语输入框');
            }

            if (scoreInputs.length === 0 && commentInputs.length === 0) {
                alert('❌ 自动填充失败：未在当前页面找到评分输入区域。\n请确保已打开学生批改详情页。');
            } else {
    ...
    function extractTextFromElement(element) {
        try {
            ...
            const excludeSelectors = [
                'script', 'style', 'button', 'input', 'textarea',
                'select', 'noscript', 'iframe', 'svg',
                '[style*="display: none"]', '[style*="visibility: hidden"]',
                '.zh-floating-ball', '#zhihuishu-ai-panel', '.zh-function-list'
            ];
    ...
    function findStandardAnswer() {
        console.log('🔍 尝试自动查找标准答案...');
        const selectors = [
            '.reference-answer',
            '.standard-answer',
            '.ans-box',
            ...
            '[class*="standard"][class*="answer"]'
        ];

        for (const selector of selectors) {
            const el = document.querySelector(selector);
            if (el && el.innerText.trim().length > 0) {
                console.log(`✅ 找到标准答案 (使用选择器: ${selector})`);
                return extractTextFromElement(el).trim();
            }
        }
        ...
        for (const div of allDivs) {
            if (div.children.length === 0 && (div.innerText.includes('标准答案') || div.innerText.includes('参考答案'))) {
                const parent = div.parentElement;
                if (parent) {
                    console.log('✅ 通过文本匹配找到标准答案区域');
                    return extractTextFromElement(parent).replace(/标准答案|参考答案|：/g, '').trim();
                }
            }
        }
                ...
                updatePanelBody(`
                    <div style="text-align:center; padding:30px; color:#666;">
                        ...
                        <div style="display: flex; gap: 10px; justify-content: center;">
                            ...
                            <button onclick="window.zhManualDOMGrading()" style="background:#4FC3F7; color:white; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-size:14px;">
                                📝 直接DOM阅卷
                            </button>
                        </div>
                    </div>
    ...
    function highlightAndCaptureArea(area) {
        ...
        updatePanelBody(`
            <div style="text-align:center; padding:30px;">
                <div style="font-size:18px; margin-bottom:10px;">📝 正在提取作业内容...</div>
                <div style="font-size:14px; color:#666;">
                    正在通过 DOM 直接获取内容
                </div>
            </div>
        ...
        setTimeout(() => {
            ...
            handleDOMGrading(area);
        }, 500);
    }

    function handleDOMGrading(area) {
        const studentAnswer = extractTextFromElement(area.element).trim();
        const standardAnswer = findStandardAnswer();

        console.log('📄 提取到学生答案:', studentAnswer.substring(0, 50) + '...');
        console.log('📄 提取到标准答案:', standardAnswer.substring(0, 50) + '...');

        updatePanelBody('<div style="text-align:center; padding:20px;">🤖 AI 正在直接通过文本分析作业...</div>');
        ...
        chrome.runtime.sendMessage({
            ...
            if (response && response.success) {
                displayHomeworkAnalysis(response.data, null, studentAnswer);
            } else {
                updatePanelBody(`
                    <div style="color:red; padding:20px;">
                        ❌ 作业分析失败: ${response ? response.error : '未知错误'}
                        <br><br>
                        <button onclick="window.zhManualScreenshot()" style="background:#FF6B6B; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">
                            尝试截图批改
                        </button>
