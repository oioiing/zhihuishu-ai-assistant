...
(function() {
    ...
    function injectStyles(){
        ...
        s.textContent = `
            ...
            .zh-loading-spinner {
                ...
                animation: zh-spin 1s ease-in-out infinite;
            }
    ...
    function createFunctionList(){
        const functions = [
            ...
            { id: 'domGrading', icon: '📝', text: 'DOM 阅卷', color: '#6C5CE7' },
    ...
    function handleFunctionClick(id, color){
        ...
        switch(id){
            ...
            case 'domGrading':
                animateRingStart(color);
                showFloatingPanel('DOM 阅卷', color, '📝 正在提取页面内容...');
                handleDomGrading();
                break;
        }
    }
    ...
    function handleDomGrading() {
        console.log('📝 开始 DOM 阅卷...');

        try {
            const standardAnswer = extractStandardAnswer();
            const studentAnswer = extractStudentAnswer();

            console.log('✅ 提取完成:', {
                hasStandard: !!standardAnswer,
                hasStudent: !!studentAnswer
            });

            if (!studentAnswer) {
                updatePanelBody('<div style="color:red; padding:20px;">❌ 未能识别到学生答案，请确保作业内容已加载。</div>');
                animateRingStop();
                return;
            }
            ...
            updatePanelBody(`
                <div style="padding:15px;">
                    <div style="margin-bottom:12px; font-weight:600; color:#4B5563;">📄 提取内容预览:</div>
                    <div style="background:#F3F4F6; padding:10px; border-radius:8px; font-size:12px; max-height:100px; overflow-y:auto; margin-bottom:15px;">
                        <strong>标准答案:</strong> ${standardAnswer || '(未找到)'}<br>
                        <strong>学生答案:</strong> ${studentAnswer.substring(0, 100)}...
                    </div>
                    <div style="text-align:center; padding:10px;">
                        <div class="zh-loading-spinner" style="margin-bottom:10px;">🤖 AI 正在批阅中...</div>
                    </div>
                </div>
            `);
    ...
    function getCleanText(element) {
        ...
        const removals = clone.querySelectorAll('button, script, style, input, .zh-floating-ball, .zh-function-list, #zhihuishu-ai-panel');
        removals.forEach(el => el.remove());
        return clone.innerText.trim();
    }

    function extractStandardAnswer() {
        // 尝试通过选择器提取 (Zhihuishu 特有)
        const selectors = ['.reference-answer', '.standard-answer', '.answer-text'];
    ...
    function extractStudentAnswer() {
        ...
        for (let selector of primaryAreas) {
            const el = document.querySelector(selector);
            if (el) {
        ...
        for (let ta of textareas) {
            if (ta.value.length > 10) return ta.value;
        }
    ...
    function displayGradingResult(data) {
        let html = `
            <div style="padding:15px; max-height:450px; overflow-y:auto;">
                <div style="background:#F0FDF4; border-radius:12px; padding:15px; border:1px solid #BBF7D0; margin-bottom:16px; text-align:center;">
                    <div style="font-size:14px; color:#166534; margin-bottom:4px;">最终得分</div>
                    <div style="font-size:36px; font-weight:800; color:#15803D;">${data.totalScore || 0}</div>
                </div>

                <div style="margin-bottom:16px;">
                    <h4 style="margin:0 0 8px 0; color:#1F2937;">📝 评分详情</h4>
                    ${(data.items || []).map(item => `
                        <div style="background:#F9FAFB; border-radius:8px; padding:10px; margin-bottom:8px; border-left:4px solid ${item.errors && item.errors.length > 0 ? '#EF4444' : '#10B981'};">
                            <div style="font-weight:600; font-size:13px; display:flex; justify-content:space-between;">
                                <span>${item.section}</span>
                                <span style="color:${item.score === item.maxScore ? '#10B981' : '#EF4444'}">${item.score}/${item.maxScore}</span>
                            </div>
                            <div style="font-size:12px; color:#6B7280; margin-top:4px;">${item.feedback}</div>
                        </div>
                    `).join('')}
                </div>

                <div style="background:#EFF6FF; border-radius:8px; padding:12px; border:1px solid #DBEAFE;">
                    <h4 style="margin:0 0 4px 0; color:#1E40AF;">💡 总体评价</h4>
                    <div style="font-size:13px; color:#1E3A8A; line-height:1.5;">${data.totalFeedback}</div>
                </div>

                <div style="margin-top:20px; display:flex; gap:10px;">
                    <button id="zh-auto-fill-btn" style="flex:1; background:#6C5CE7; color:white; border:none; padding:10px; border-radius:8px; cursor:pointer; font-weight:600;">✨ 自动填写成绩</button>
                </div>
            </div>
        `;
        ...
        document.getElementById('zh-auto-fill-btn').onclick = () => {
            autoFillZhihuishu(data.totalScore, data.totalFeedback);
        };
    }

    function autoFillZhihuishu(score, comment) {
        try {
            // 尝试多种可能的选择器
            const scoreSelectors = [
                ...
                'input.el-input__inner[type="text"]' // 有些地方用 text 模拟 number
            ];
            ...
            if (scoreFilled || commentFilled) {
                if (btn) {
                    btn.textContent = '✅ 已成功填入' + (scoreFilled && commentFilled ? '' : (scoreFilled ? '(仅分数)' : '(仅评语)'));
                    btn.style.background = '#10B981';
                }
                console.log('✅ 自动填充成功:', { scoreFilled, commentFilled });
            } else {
                throw new Error('未找到分值输入框或评语输入框，请确保已打开批阅窗口。');
            }
        } catch (error) {
