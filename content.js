...
(function() {
    ...
    function displayHomeworkAnalysis(analysisData, croppedImage) {
        const html = `
            <div style="max-height:500px; overflow-y:auto; padding:10px;">
                ...
                ` : `
                <div style="text-align:center; margin-bottom:16px; padding:12px; background:#f0f7ff; border-radius:8px; border: 1px dashed #2196F3; margin-bottom:16px;">
                    <div style="font-weight:600; color:#1976D2; margin-bottom:4px;">📄 DOM 文本解析模式</div>
                    <div style="font-size:12px; color:#666;">直接从网页提取文字，准确度更高</div>
                </div>
                `}
    ...
    function displayHomeworkAreaSelection(areas) {
        let html = `
            <div style="padding:16px;">
                <div style="margin-bottom:16px; padding:12px; background:#e3f2fd; border-radius:8px; border-left:4px solid #2196F3;">
                    ...
                    <p style="margin:0; font-size:13px; color:#555;">请选择批改方式：<b>DOM 批改</b>更准确，<b>截图批改</b>更直观。</p>
                </div>
        ...
        setTimeout(() => {
            ...
            const screenBtns = document.querySelectorAll('.zh-screenshot-grade-btn');

            domBtns.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const idx = e.currentTarget.getAttribute('data-index');
                    analyzeAreaWithDOM(areas[idx]);
                });
            });
            ...
            screenBtns.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const idx = e.currentTarget.getAttribute('data-index');
                    highlightAndCaptureArea(areas[idx]);
                });
            });
        }, 100);
    ...
    function analyzeAreaWithDOM(area) {
        ...
        updatePanelBody(`
            <div style="text-align:center; padding:30px;">
                <div style="font-size:18px; margin-bottom:10px;">📄 正在提取并分析 DOM 文本...</div>
                <div style="font-size:14px; color:#666;">
                    正在从网页直接抓取文字内容进行 AI 批改
                </div>
            </div>
        `);

        const text = area.element.innerText || area.element.textContent || '';

        setTimeout(() => {
            highlightOverlay.remove();

            if (!text.trim()) {
                updatePanelBody(`<div style="color:red; padding:20px;">❌ 未能在选定区域提取到有效文本，请尝试使用截图批改。</div>`);
                animateRingStop();
                return;
            }
            ...
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    updatePanelBody(`<div style="color:red; padding:20px;">❌ DOM 批改失败: ${response ? response.error : '未知错误'}</div>`);
                }
                animateRingStop();
            });
        }, 500);
