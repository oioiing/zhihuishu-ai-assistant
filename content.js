...
(function() {
    ...
    function captureSelectedArea(rect, domContent) {
        updatePanelBody(`
                    ...
                    .then(croppedImage => {
                        ...
                        chrome.runtime.sendMessage({
                            ...
                            if (analysisResponse && analysisResponse.success) {
                                displayHomeworkAnalysis(analysisResponse.data, croppedImage, domContent);
                            } else {
                                updatePanelBody(`
    ...
    function displayHomeworkAnalysis(analysisData, croppedImage, domContent) {
        const html = `
            <div style="max-height:500px; overflow-y:auto; padding:10px;">
                <!-- 预览内容 -->
                <div style="text-align:center; margin-bottom:16px; padding:12px; background:#f8f9fa; border-radius:8px;">
                    <div style="font-weight:600; margin-bottom:8px;">${croppedImage ? '📸 截图内容' : '📄 DOM 提取内容'}</div>
                    ${croppedImage ? `<img src="${croppedImage}" style="max-width:100%; max-height:200px; border:1px solid #ddd; border-radius:4px;">` :
                      `<div style="font-size:12px; color:#666; text-align:left; max-height:100px; overflow-y:auto; background:#fff; padding:8px; border-radius:4px; border:1px solid #eee;">${domContent}</div>`}
                </div>
    ...
    function displayHomeworkAreaSelection(areas) {
        let html = `
            <div style="padding:16px;">
                <div style="margin-bottom:16px; padding:12px; background:#e3f2fd; border-radius:8px; border-left:4px solid #2196F3;">
                    ...
                    <p style="margin:0; font-size:13px; color:#555;">点击区域进行截图批改，或使用 DOM 快速阅卷</p>
                </div>
        ...
        areas.forEach((area, index) => {
            ...
            html += `
                <div class="homework-area-item-container" style="margin-bottom:16px; border:1px solid #eee; border-radius:12px; overflow:hidden; background:#fff; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    ...
                    <div style="display:flex; border-top:1px solid #eee;">
                        <button class="dom-grade-btn" data-index="${index}" style="
                            flex:1;
                            padding:10px;
                            background:#fff;
                            color:#2196F3;
                            ...
                            gap:4px;
                        ">✨ DOM 快速阅卷</button>
                        <button class="screenshot-grade-btn" data-index="${index}" style="
                            flex:1;
                            padding:10px;
                            background:#fff;
                            color:#666;
        ...
        setTimeout(() => {
            ...
            const domBtns = document.querySelectorAll('.dom-grade-btn');
            domBtns.forEach((btn, index) => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    analyzeAreaWithDOM(areas[index]);
                });
            });

            const screenshotBtns = document.querySelectorAll('.screenshot-grade-btn');
    ...
    function analyzeAreaWithDOM(area) {
        console.log('✨ 准备使用 DOM 内容进行阅卷...');
        ...
        updatePanelBody(`
            <div style="text-align:center; padding:30px;">
                <div style="font-size:18px; margin-bottom:10px;">🤖 正在提取页面内容并批改...</div>
                <div style="font-size:14px; color:#666;">
                    正在直接分析 DOM 内容，无需截图 OCR
                </div>
            </div>
        `);

        // 提取文本
        const domContent = area.element.innerText;
        ...
        setTimeout(() => {
            ...
            chrome.runtime.sendMessage({
                ...
                if (analysisResponse && analysisResponse.success) {
                    displayHomeworkAnalysis(analysisResponse.data, null, domContent);
                } else {
                    updatePanelBody(`
                        <div style="color:red; padding:20px;">
                            ❌ 作业分析失败: ${analysisResponse ? analysisResponse.error : '未知错误'}
