...
(function() {
    ...
    function handleDOMGrading() {
        ...
        try {
            // 提取题目和答案内容
            const standardAnswerEl = document.querySelector('.reference-answer, .standard-answer, .ans-box, [class*="answer-content"]');
            const studentAnswerEl = document.querySelector('.break-all.break-words, .answer-box, .markdown-latex-container, .evaluation-content, [class*="student-work"]');

            if (!studentAnswerEl) {
                throw new Error('未检测到学生作答内容，请确保已进入作业批阅页面。');
            }

            // 清洗提取的内容
            const cleanExtract = (el) => {
                if (!el) return '';
                const clone = el.cloneNode(true);
                const ignore = clone.querySelectorAll('button, script, style, input, .hidden, [style*="display: none"]');
                ignore.forEach(e => e.remove());
                return clone.innerText.trim();
            };

            const data = {
                standardAnswer: cleanExtract(standardAnswerEl),
                studentAnswer: cleanExtract(studentAnswerEl),
                url: window.location.href,
                title: document.title
            };

            console.log('✅ 内容提取成功:', data);
            ...
            chrome.runtime.sendMessage({
                ...
                if (res && res.success) {
                    ...
                    console.error('❌ 阅卷失败:', res);
                    updatePanelBody(`<div style="color:red; padding:10px;">❌ 阅卷失败: ${res ? res.error : '未知错误'}</div>`);
                }
                animateRingStop();
            });
            ...
            updatePanelBody(`<div style="color:red; padding:10px;">❌ 提取失败: ${error.message}</div>`);
            animateRingStop();
        }
    }

    function displayDOMGradingResult(result) {
        const score = result.score || 0;
        const feedback = result.feedback || '未提供评语';

        const html = `
            <div style="padding:10px;">
                <div style="margin-bottom:15px; padding:12px; background:#f0f7ff; border-radius:8px; border-left:4px solid #673AB7;">
                    <div style="font-weight:600; color:#673AB7; margin-bottom:4px;">📊 AI 评分建议</div>
                    <div style="font-size:24px; font-weight:bold; color:#333;">${score} <span style="font-size:14px; font-weight:normal; color:#666;">分</span></div>
                </div>

                <div style="margin-bottom:15px;">
                    <div style="font-weight:600; margin-bottom:8px; color:#333;">📝 AI 评语</div>
                    <div style="padding:12px; background:#f9fafb; border:1px solid #e5e7eb; border-radius:8px; font-size:13px; line-height:1.6; white-space:pre-wrap;">${feedback}</div>
                </div>

                <div style="text-align:center; margin-top:20px;">
                    <button id="zh-auto-fill-btn" style="background:#673AB7; color:white; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600; font-size:14px; transition:all 0.2s;">
                        ✨ 一键填充到页面
                    </button>
                </div>
            </div>
    ...
    function autoFillGrading(score, feedback) {
        console.log('✨ 开始自动填充阅卷结果:', { score, feedback });

        try {
            // 查找分数输入框
            const scoreInput = document.querySelector('input[placeholder*="成绩"], input.el-input__inner[type="number"], .score-input input, input[aria-label*="成绩"]');
            // 查找评语文本框
            const feedbackTextarea = document.querySelector('textarea.el-textarea__inner, textarea[placeholder*="评语"], .comment-box textarea, textarea[aria-label*="评语"]');
            ...
            if (feedbackTextarea) {
                ...
                console.warn('⚠️ 未找到评语文本框');
            }

            if (!scoreInput && !feedbackTextarea) {
                alert('自动填充失败：未在页面上找到对应的评分输入框或评语框。');
            }
        } catch (error) {
            console.error('❌ 自动填充出错:', error);
            alert('自动填充出错: ' + error.message);
        }
    }
