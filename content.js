...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            ...
            { id: 'domGrading', icon: '📝', text: 'DOM 阅卷', color: '#4A90E2' },
    ...
    // DOM 阅卷功能逻辑
    // ==========================================
    function handleDOMGrading(color) {
        console.log('📝 开始 DOM 阅卷提取...');
        ...
        try {
            ...
            if (!studentAnswer) {
                updatePanelBody('<div style="color:red; padding:20px;">❌ 未检测到学生答案，请确保页面已加载完成。</div>');
                animateRingStop();
                return;
            }

            console.log('✅ 提取到学生答案长度:', studentAnswer.length);
            console.log('✅ 提取到标准答案长度:', standardAnswer.length);

            const textToAnalyze = `【标准答案】\n${standardAnswer || '未检测到标准答案'}\n\n【学生答案】\n${studentAnswer}`;
            ...
            chrome.runtime.sendMessage({
                ...
                if (res && res.success) {
                    displayDOMGradingResult(res.data, color);
                } else {
                    updatePanelBody(`<div style="color:red; padding:20px;">❌ AI 分析失败: ${res ? res.error : '未知错误'}</div>`);
                }
                animateRingStop();
            });
        } catch (error) {
            console.error('❌ DOM 阅卷提取失败:', error);
            updatePanelBody(`<div style="color:red; padding:20px;">❌ 提取失败: ${error.message}</div>`);
            animateRingStop();
        }
    }
    ...
    function cleanExtractedText(element) {
        ...
        const junkSelectors = ['button', 'script', 'style', 'input', '.ai-helper', '.tool-bar'];
    ...
    function displayDOMGradingResult(analysisData, color) {
        ...
        if (typeof analysisData === 'object') {
            score = analysisData.totalScore || analysisData.score || 0;
            feedback = analysisData.totalFeedback || analysisData.feedback || JSON.stringify(analysisData);
        } else {
            feedback = analysisData;
            const scoreMatch = feedback.match(/总分：(\d+)/) || feedback.match(/分数：(\d+)/);
        ...
        headerDiv.style.cssText = `background:#f8f9fa; border-radius:8px; border-left:4px solid ${color}; padding:12px; margin-bottom:16px;`;
        headerDiv.innerHTML = `<h4 style="margin:0 0 8px 0; color:${color};">🤖 AI 阅卷结果</h4><div style="font-size:24px; font-weight:bold; color:#ef4444; margin-bottom:8px;">得分：${score}</div>`;

        const feedbackDiv = document.createElement('div');
        feedbackDiv.className = 'zh-scroll';
        feedbackDiv.style.cssText = 'background:white; border:1px solid #e5e7eb; border-radius:8px; padding:16px; white-space:pre-wrap; font-size:14px; line-height:1.6; max-height:300px; overflow-y:auto; margin-bottom:16px;';
        feedbackDiv.textContent = feedback; // Use textContent for safety

        const btnDiv = document.createElement('div');
        btnDiv.style.textAlign = 'center';
        const fillBtn = document.createElement('button');
        fillBtn.id = 'zh-auto-fill-btn';
        fillBtn.style.cssText = `background:${color}; color:white; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600; transition:all 0.2s;`;
        fillBtn.textContent = '✨ 一键填充';

        btnDiv.appendChild(fillBtn);
        ...
        fillBtn.onclick = (e) => {
            e.stopPropagation();
            if (autoFillGradeAndComment(score, feedback)) {
                fillBtn.textContent = '✅ 填充完成';
                fillBtn.style.background = '#10b981';
                fillBtn.disabled = true;
            } else {
                alert('未找到分数输入框或评语框，请确保页面已切换到批改模式。');
            }
        };
    }
