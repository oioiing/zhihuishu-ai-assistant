...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            ...
            { id: 'domGrading', icon: '📝', text: 'DOM 阅卷', color: '#4CAF50' },
    ...
    function handleFunctionClick(id, color){
        ...
        switch(id){
            ...
            case 'domGrading':
                animateRingStart(color);
                showFloatingPanel('DOM 自动阅卷', color, '🔍 正在读取页面内容...');
                handleDOMGrading();
                break;
    ...
    function extractGradingData() {
        console.log('🔍 开始从 DOM 提取阅卷数据...');

        // 1. 尝试提取标准答案
        const standardAnswerSelectors = [
            '.reference-answer',
            '.standard-answer',
            '.ans-box',
        ...
        for (const selector of standardAnswerSelectors) {
            ...
            if (el && el.innerText.trim()) {
                standardAnswer = el.innerText.trim();
                console.log('✅ 找到标准答案:', standardAnswer.substring(0, 30) + '...');
                break;
            }
        }
        ...
        for (const selector of studentAnswerSelectors) {
            ...
            if (el && el.innerText.trim()) {
                ...
                console.log('✅ 找到学生答案:', studentAnswer.substring(0, 30) + '...');
                break;
            }
        }

        // 3. 提取题目（可选，用于辅助分析）
        const questionSelectors = ['.homework-content', '.question-box', '.subject_describe'];
        let question = '';
    ...
    function autoFillGradeAndComment(score, feedback) {
        console.log(`🚀 开始自动填充成绩: ${score} 和评语`);
    ...
    function handleDOMGrading() {
        console.log('🤖 开始 DOM 自动阅卷流程...');

        const gradingData = extractGradingData();

        if (!gradingData.studentAnswer) {
            updatePanelBody('<div style="color:red; padding:20px;">❌ 未能提取到学生答案，请确保已进入批改页面。</div>');
            animateRingStop();
            return;
        }
        ...
        updatePanelBody(`
            <div style="padding:20px;">
                <div style="margin-bottom:15px; font-weight:600;">📝 提取到的数据:</div>
                <div style="font-size:12px; background:#f5f5f5; padding:10px; border-radius:8px; max-height:150px; overflow-y:auto; margin-bottom:15px;">
                    <strong>标准答案:</strong> ${gradingData.standardAnswer || '未找到'}<br><br>
                    <strong>学生答案:</strong> ${gradingData.studentAnswer}
                </div>
                <div style="text-align:center;">🤖 AI 正在阅卷中...</div>
            </div>
        `);
        ...
        chrome.runtime.sendMessage({
            ...
            if (res && res.success) {
                    ...
                    <div style="padding:20px;">
                        <div style="background:#f0f9ff; padding:15px; border-radius:12px; border:1px solid #bae6fd; margin-bottom:20px;">
                            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                                <span style="font-weight:600; color:#0369a1;">📊 AI 阅卷结果</span>
                                <span style="background:#0ea5e9; color:white; padding:2px 10px; border-radius:20px; font-size:18px; font-weight:bold;">${data.score}分</span>
                            </div>
                            <div style="font-size:14px; color:#334155; line-height:1.6; white-space:pre-wrap;">${data.feedback}</div>
                        </div>
                        <div style="text-align:center;">
                            <button id="zh-auto-fill-btn" style="background:#66BB6A; color:white; border:none; padding:10px 30px; border-radius:8px; cursor:pointer; font-weight:600; transition:all 0.2s;">
                                📝 自动填充到页面
                            </button>
                        </div>
                    </div>
                ...
                document.getElementById('zh-auto-fill-btn').onclick = (e) => {
                    e.stopPropagation();
                    ...
                    if (success) {
                        ...
                        alert('❌ 自动填充失败，请检查输入框是否可见。');
                    }
                };
            } else {
                updatePanelBody(`<div style="color:red; padding:20px;">❌ 阅卷失败: ${res ? res.error : '未知错误'}</div>`);
            }
            animateRingStop();
        });
    }
    ...
    function displayHomeworkAnalysis(analysisData, croppedImage) {
                ...
                <div style="background:#fff; border-radius:8px; border:1px solid #e5e7eb;">
                    ...
                    <div style="padding:16px; white-space:pre-wrap; line-height:1.6; font-size:14px;">
                        ${typeof analysisData === 'object' ?
                            `**总分：${analysisData.totalScore}分**\n\n${analysisData.totalFeedback}` :
                            analysisData}
                    </div>
                </div>
        ...
        window.zhSaveAnalysis = () => {
            // 创建下载链接
            const report = typeof analysisData === 'object' ?
