...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            { id: 'domGrading', icon: '🤖', text: 'DOM 智能阅卷', color: '#6366F1' },
    ...
    function handleFunctionClick(id, color){
        ...
        switch(id){
            case 'domGrading':
                ...
                showFloatingPanel('DOM 智能阅卷', color, '🤖 正在提取页面内容...');
    ...
    function handleDOMGrading(color) {
        ...
        try {
            ...
            if (!studentAnswer) {
                updatePanelBody(`
                    <div style="text-align:center; padding:20px; color:#ef4444;">
                        ...
                        <h4 style="margin:0 0 8px 0;">未找到学生答案</h4>
                        <div style="font-size:13px; color:#64748b;">
                            请确保已打开学生作业详情页面，<br>或者尝试使用"截图批改"。
                        </div>
                    </div>
                `);
                animateRingStop();
            ...
            updatePanelBody(`
                <div style="text-align:center; padding:20px;">
                    <div style="margin-bottom:12px;">🤖 AI 正在阅卷中...</div>
                    <div style="font-size:12px; color:#94a3b8;">内容长度: ${homeworkText.length} 字符</div>
                </div>
            `);
            ...
            console.error('❌ DOM 阅卷异常:', error);
            updatePanelBody(`<div style="color:red; padding:20px;">❌ 发生异常: ${error.message}</div>`);
            animateRingStop();
        }
    }

    function displayDOMGradingResult(data, color) {
        const html = `
            <div style="max-height:500px; overflow-y:auto; padding:10px;">
                <div style="background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0; margin-bottom:16px; overflow:hidden;">
                    <div style="background:${color}; color:white; padding:12px 16px; font-weight:600; display:flex; justify-content:space-between; align-items:center;">
                        <span>📊 评分结果</span>
                        <span style="font-size:20px;">${data.totalScore} 分</span>
                    </div>
                    <div style="padding:16px; white-space:pre-wrap; line-height:1.6; font-size:14px; color:#334155;">
                        ${data.totalFeedback}
                    </div>
                </div>
        ...
        if (fillBtn) {
            fillBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                const success = autoFillGrade(data.totalScore, data.totalFeedback);
                if (success) {
                    fillBtn.textContent = '✅ 填充完成';
                    fillBtn.style.background = '#10b981';
                    fillBtn.disabled = true;
                } else {
                    alert('未找到输入框，请确保处于批改编辑状态');
                }
            });
        }
    }
        ...
        for (const selector of studentSelectors) {
            ...
            for (const el of elements) {
                if (isValidHomeworkArea(el)) {
                    ...
                    if (text.length > 5) {
                        console.log(`✅ 找到学生答案 (${selector}):`, text.substring(0, 50));
                        return text;
                    }
                }
            }
        ...
        for (const selector of standardSelectors) {
            ...
            for (const el of elements) {
                if (isValidHomeworkArea(el)) {
                    const text = extractCleanText(el);
                    if (text.length > 5) {
                        console.log(`✅ 找到标准答案 (${selector}):`, text.substring(0, 50));
                        return text;
                    }
                }
            }
    ...
    function extractCleanText(element) {
        ...
        // 移除不需要的子元素
        const toRemove = clone.querySelectorAll('button, script, style, input, textarea, .zh-floating-ball');
    ...
    // 自动填充成绩和评语
    function autoFillGrade(score, feedback) {
        console.log('📝 开始自动填充成绩和评语:', { score, feedbackLength: feedback.length });
        ...
        return !!(scoreInput || feedbackTextarea);
    }
