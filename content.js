...
(function() {
    ...
    /**
     * 显示 DOM 阅卷的 AI 分析结果
     * @param {string} analysisData AI 返回的分析内容
     * @param {string} extractedText 提取的原始文本
     */
    function displayDOMAnalysisResult(analysisData, extractedText) {
        const panelBody = document.getElementById('zh-panel-body');
        if (!panelBody) return;

        // 使用 DOM API 构建界面，防止 XSS 并确保格式
        panelBody.innerHTML = '';

        const container = document.createElement('div');
        container.style.cssText = 'max-height:500px; overflow-y:auto; padding:10px;';

        // 1. 提取的文本内容预览
        const textPreview = document.createElement('div');
        textPreview.style.cssText = 'margin-bottom:16px; padding:12px; background:#f8f9fa; border-radius:8px; border: 1px solid #eef0f2;';

        const previewTitle = document.createElement('div');
        previewTitle.style.cssText = 'font-weight:600; margin-bottom:8px; color: #4b5563; font-size: 13px;';
        previewTitle.textContent = '📝 提取的网页文本';
        textPreview.appendChild(previewTitle);

        const textContent = document.createElement('div');
        textContent.style.cssText = 'font-size:12px; color:#6b7280; max-height:80px; overflow-y:auto; white-space:pre-wrap; background:#fff; padding:8px; border:1px solid #e5e7eb; border-radius:4px; line-height: 1.4;';
        textContent.textContent = extractedText;
        textPreview.appendChild(textContent);
        container.appendChild(textPreview);

        // 2. AI 分析结果卡片
        const resultCard = document.createElement('div');
        resultCard.style.cssText = 'background:#fff; border-radius:12px; border:1px solid #e5e7eb; overflow:hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);';

        const resultHeader = document.createElement('div');
        resultHeader.style.cssText = 'background:#2196F3; color:white; padding:14px 16px; font-weight:600; font-size: 15px; display: flex; align-items: center; gap: 8px;';
        resultHeader.innerHTML = '<span>🤖</span> AI 智能批改建议';
        resultCard.appendChild(resultHeader);

        const resultBody = document.createElement('div');
        resultBody.style.cssText = 'padding:20px; white-space:pre-wrap; line-height:1.7; font-size:14px; color: #1f2937;';
        resultBody.textContent = analysisData;
        ...
        footer.style.cssText = 'margin-top:20px; text-align:center; display:flex; gap:12px; justify-content:center; padding-bottom: 10px;';

        const backBtn = document.createElement('button');
        backBtn.textContent = '🎯 返回列表';
        backBtn.style.cssText = 'background:#f3f4f6; color:#374151; border:1px solid #d1d5db; padding:10px 20px; border-radius:8px; cursor:pointer; font-size:13px; font-weight: 500; transition: all 0.2s;';
        backBtn.onmouseover = () => backBtn.style.background = '#e5e7eb';
        backBtn.onmouseout = () => backBtn.style.background = '#f3f4f6';
        backBtn.onclick = () => window.zhBackToDetection();

        const saveBtn = document.createElement('button');
        saveBtn.textContent = '💾 保存报告';
        saveBtn.style.cssText = 'background:#2196F3; color:white; border:none; padding:10px 20px; border-radius:8px; cursor:pointer; font-size:13px; font-weight: 500; transition: all 0.2s;';
        saveBtn.onmouseover = () => saveBtn.style.background = '#1e88e5';
        saveBtn.onmouseout = () => saveBtn.style.background = '#2196F3';
        saveBtn.onclick = () => window.zhSaveAnalysisResult(analysisData);
        ...
        // 注册全局保存函数
        window.zhSaveAnalysisResult = (data) => {
            const reportContent = `智慧树 AI 助教 - 作业批改报告 (DOM 模式)\n生成时间: ${new Date().toLocaleString()}\n\n${data}\n\n--- 原始提取文本 ---\n${extractedText}`;
            const blob = new Blob([reportContent], { type: 'text/plain;charset=utf-8' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `AI阅卷报告_DOM_${new Date().toISOString().slice(0,10)}.txt`;
            a.click();
    ...
    function displayHomeworkAreaSelection(areas) {
            ...
            html += `
                ...
                " onmouseover="this.style.borderColor='#2196F3'; this.style.background='#f0f7ff';"
                   onmouseout="this.style.borderColor='#e0e0e0'; this.style.background='white';">
                    ...
                    ">${preview}</div>
                    <div style="display:flex; gap:8px;">
                        <button class="dom-grade-btn" data-index="${index}" style="flex:1; background:#2196F3; color:white; border:none; padding:6px; border-radius:4px; cursor:pointer; font-size:11px;">
                            🤖 DOM 自动阅卷
                        </button>
                        <button class="screenshot-grade-btn" data-index="${index}" style="flex:1; background:#FF6B6B; color:white; border:none; padding:6px; border-radius:4px; cursor:pointer; font-size:11px;">
                            📸 截图阅卷
                        </button>
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            // 专门的 DOM 阅卷按钮
            document.querySelectorAll('.dom-grade-btn').forEach(btn => {
                btn.onclick = (e) => {
                    e.stopPropagation();
            ...
            // 专门的截图阅卷按钮
            document.querySelectorAll('.screenshot-grade-btn').forEach(btn => {
                btn.onclick = (e) => {
                    e.stopPropagation();
                    const index = btn.getAttribute('data-index');
    ...
    function extractHomeworkDataFromDOM(element) {
        console.log('🔍 正在从 DOM 提取作业内容...');

        // 克隆元素以避免副作用
        const clone = element.cloneNode(true);

        // 移除不必要的干扰元素（如按钮、脚本、隐藏元素等）
        const selectorsToRemove = [
            'script', 'style', 'button', '.btn',
            '.ui-dialog', '.pop-up', '[style*="display: none"]',
            '.hidden', '.ignore'
        ];
        ...
        // 常见的题目和选项结构
        const structures = clone.querySelectorAll('.question-title, .question-content, .option-list, .answer-analysis');
        ...
        console.log('✅ DOM 内容提取完成，长度:', cleanedText.length);
        return cleanedText;
    }

    /**
     * 处理 DOM 自动阅卷
     * @param {Object} area 识别到的作业区域对象
     */
    function handleDOMGrading(area) {
        ...
        // 延迟移除高亮
        setTimeout(() => highlightOverlay.remove(), 1000);

        // 更新面板状态
        updatePanelBody(`
            <div style="text-align:center; padding:30px;">
                <div style="font-size:18px; margin-bottom:10px;">🤖 正在提取 DOM 内容并分析...</div>
                <div style="font-size:14px; color:#666;">
                    正在获取网页原始文本以获得最高准确度
                </div>
            </div>
        ...
        try {
            ...
            if (!extractedText || extractedText.length < 5) {
                throw new Error('未能从选定区域提取到足够的文字内容');
            }
            ...
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    updatePanelBody(`
                        <div style="color:red; padding:20px;">
                            ❌ 分析失败: ${response ? response.error : '未知错误'}
                            <br><br>
                            <button onclick="window.zhManualScreenshot()" style="background:#FF6B6B; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">
                                尝试截图批改
                            </button>
