...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            { id: 'domGrading', icon: '📝', text: 'DOM 文本阅卷', color: '#66BB6A' },
    ...
    function handleFunctionClick(id, color){
        ...
        switch(id){
            case 'domGrading':
                ...
                showFloatingPanel('DOM 文本阅卷', color, '🔍 正在提取网页作答内容...');
                handleDOMTextGrading();
                break;
    ...
    function extractZhihuishuHomeworkData() {
        console.log('🔍 开始从 DOM 提取作业数据...');

        // 1. 提取标准答案
        let standardAnswer = '';
        const standardAnswerEl = document.querySelector('.reference-answer, .standard-answer, [class*="standardAnswer"]');
        if (standardAnswerEl) {
            standardAnswer = standardAnswerEl.innerText.trim();
        }
        ...
        for (const selector of studentAnswerSelectors) {
            ...
            if (el && el.innerText.trim().length > 0) {
                studentAnswer = el.innerText.trim();
                break;
            }
        }
        ...
        // 3. 提取作业要求/题目
        let requirements = '';
        const reqEl = document.querySelector('.homework-content, .question-title, [class*="homeworkContent"]');
        if (reqEl) {
            requirements = reqEl.innerText.trim();
        }
        ...
        return { studentAnswer, standardAnswer, requirements };
    }

    /**
     * 自动填充成绩和评语
     */
    function autoFillGradingResult(score, comment) {
        console.log(`✍️ 开始自动填充: 分数=${score}`);

        try {
            // 1. 填充分数
            const scoreInput = document.querySelector('input[placeholder*="成绩"], input[class*="score-input"]');
            if (scoreInput) {
            ...
            return !!(scoreInput || commentTextarea);
        } catch (error) {
     ...
     * 处理 DOM 文本阅卷流程
     */
    function handleDOMTextGrading() {
        console.log('🚀 启动 DOM 文本阅卷...');
        const data = extractZhihuishuHomeworkData();

        if (!data.studentAnswer) {
            updatePanelBody('<div style="color:red;">❌ 未检测到学生作答内容，请确保页面已加载完成。</div>');
            animateRingStop();
            return;
        }

        updatePanelBody('<div style="text-align:center; padding:20px;">🤖 AI 正在根据网页内容进行批改...</div>');
        ...
        chrome.runtime.sendMessage({
            ...
            if (response && response.success) {
                ...
                // 尝试解析分数
                let score = 0;
                const scoreMatch = aiResult.match(/总分：(\d+)/) || aiResult.match(/(\d+)分/);
                ...
                container.innerHTML = `
                    <div style="background:#f0f9ff; padding:12px; border-radius:8px; border-left:4px solid #0ea5e9; margin-bottom:15px;">
                        <h4 style="margin:0 0 4px 0; color:#0369a1;">📊 阅卷预览</h4>
                        <div style="font-size:14px;">建议得分：<span style="font-weight:bold; color:#ef4444; font-size:18px;">${score}</span></div>
                    </div>
                    <div style="background:#fff; border-radius:8px; border:1px solid #e5e7eb;">
                        <div style="background:#4FC3F7; color:white; padding:10px; border-radius:8px 8px 0 0; font-weight:600; font-size:14px;">
                            🤖 AI 批改详情
                        </div>
                        <div id="zh-ai-result-content" style="padding:15px; white-space:pre-wrap; line-height:1.6; font-size:13px; color:#374151; max-height:300px; overflow-y:auto;"></div>
                    </div>
                    <div style="margin-top:15px; text-align:center;">
                        <button id="zh-auto-fill-btn" style="background:#66BB6A; color:white; border:none; padding:10px 20px; border-radius:8px; cursor:pointer; font-weight:600; width:100%;">
                            ✨ 自动填充到网页
                        </button>
                    </div>
                `;
                ...
                if (body) {
                    ...
                    // 安全地设置 AI 结果文本
                    document.getElementById('zh-ai-result-content').textContent = aiResult;
                }
                ...
                if (fillBtn) {
                    fillBtn.onclick = (e) => {
                        e.stopPropagation();
                        const success = autoFillGradingResult(score, aiResult);
                        if (success) {
                            fillBtn.textContent = '✅ 已填充';
                            fillBtn.style.background = '#9e9e9e';
                            fillBtn.disabled = true;
                        } else {
                            alert('填充失败，请检查是否处于批改界面');
                        }
                    };
                }
            } else {
                updatePanelBody(`<div style="color:red;">❌ 批改失败: ${response ? response.error : '未知错误'}</div>`);
            }
            animateRingStop();
        });
    }
