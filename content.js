...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            { id: 'domGrading', icon: '📝', text: 'DOM 阅卷', color: '#673AB7' },
    ...
    function handleFunctionClick(id, color){
        ...
        switch(id){
            case 'domGrading':
                ...
                showFloatingPanel('DOM 自动阅卷', color, '📝 正在提取页面答案...');
                handleDomGrading();
                break;
    ...
    function handleDomGrading() {
        ...
        try {
            ...
            console.log('✅ DOM 数据提取成功:', homeworkData);

            if (!homeworkData.standardAnswer || !homeworkData.studentAnswer) {
                updatePanelBody(`
                    <div style="color:red; padding:20px;">
                        ⚠️ 未能完整提取答案内容：<br>
                        - 标准答案: ${homeworkData.standardAnswer ? '✅' : '❌'}<br>
                        - 学生答案: ${homeworkData.studentAnswer ? '✅' : '❌'}<br><br>
                        请确保页面已完全加载，且处于批改界面。
                    </div>
                `);
                animateRingStop();
            ...
            updatePanelBody('<div style="text-align:center; padding:20px;">🤖 AI 正在对比答案并评分...</div>');
            ...
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    updatePanelBody(`
                        <div style="color:red; padding:20px;">
                            ❌ AI 评分失败: ${response ? response.error : '未知错误'}
                        </div>
                    `);
                }
                animateRingStop();
            ...
            updatePanelBody(`<div style="color:red; padding:20px;">❌ 阅卷失败: ${error.message}</div>`);
            animateRingStop();
        }
    }
    ...
    function extractHomeworkDataFromDom() {
        // 1. 提取题目
        const questionSelectors = ['.homework-content', '.question-box', '.title', '.homework-title'];
        let question = '';
        for (const s of questionSelectors) {
            const el = document.querySelector(s);
            if (el) {
                question = cleanExtractedText(el);
                break;
            }
        }

        // 2. 提取标准答案
        const standardSelectors = ['.reference-answer', '.standard-answer', '.ans-box', '[class*="answer-content"]'];
        let standardAnswer = '';
        for (const s of standardSelectors) {
            const el = document.querySelector(s);
            if (el) {
                standardAnswer = cleanExtractedText(el);
                break;
            }
        }

        // 3. 提取学生答案
        const studentSelectors = ['.break-all.break-words', '.answer-box', '.markdown-latex-container', '.evaluation-content', '[class*="student-work"]'];
        let studentAnswer = '';
        for (const s of studentSelectors) {
            const el = document.querySelector(s);
            if (el) {
                studentAnswer = cleanExtractedText(el);
                break;
            }
        }

        return { question, standardAnswer, studentAnswer };
    }
    ...
    function cleanExtractedText(element) {
        ...
        // 移除不必要的子元素
        const junk = clone.querySelectorAll('button, script, style, input, .zh-floating-ball');
    ...
    function displayDomGradingResult(data) {
        ...
        scoreHeader.style.cssText = 'background:#f3e8ff; border-left:4px solid #673AB7; padding:12px; border-radius:4px; margin-bottom:16px;';

        const scoreValue = document.createElement('div');
        scoreValue.style.cssText = 'font-size:24px; font-weight:bold; color:#673AB7;';
        scoreValue.textContent = `${data.score} 分`;

        const scoreLabel = document.createElement('div');
        scoreLabel.style.cssText = 'font-size:13px; color:#6b7280; margin-top:4px;';
        scoreLabel.textContent = 'AI 智能评分结果';
        ...
        feedbackTitle.style.cssText = 'margin:0 0 8px 0; color:#1e293b;';
        feedbackTitle.textContent = '💡 批改评语';
        container.appendChild(feedbackTitle);

        const feedbackBody = document.createElement('div');
        feedbackBody.style.cssText = 'background:#f8fafc; padding:12px; border-radius:8px; font-size:14px; white-space:pre-wrap; margin-bottom:16px;';
        feedbackBody.textContent = data.feedback;
        container.appendChild(feedbackBody);

        // 细节区
        if (data.details && data.details.length > 0) {
            const detailsTitle = document.createElement('h4');
            detailsTitle.style.cssText = 'margin:0 0 8px 0; color:#1e293b;';
            detailsTitle.textContent = '📌 评分细节';
            container.appendChild(detailsTitle);

            const detailsList = document.createElement('div');
            detailsList.style.cssText = 'display:flex; flex-direction:column; gap:8px; margin-bottom:16px;';

            data.details.forEach(d => {
                const item = document.createElement('div');
                item.style.cssText = 'background:#fff; border:1px solid #e2e8f0; padding:8px 12px; border-radius:6px; font-size:13px;';

                const pointSpan = document.createElement('span');
                pointSpan.style.cssText = 'font-weight:600; color:#673AB7;';
                pointSpan.textContent = `${d.point}: `;

                const commentText = document.createTextNode(d.comment);

                item.appendChild(pointSpan);
        ...
        fillBtn.style.cssText = 'background:#673AB7; color:white; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600; transition:all 0.2s;';
        fillBtn.textContent = '🚀 填充成绩与评语';
        ...
        if (fillBtn) {
            fillBtn.onclick = (e) => {
                e.stopPropagation();
                autoFillGradeAndComment(data.score, data.feedback);
    ...
    function autoFillGradeAndComment(score, feedback) {
        console.log('🚀 开始自动填充成绩和评语:', { score, feedback });
