...
(function() {
    ...
    function createFunctionList(){
        const functions = [
            { id: 'domGrading', icon: '📝', text: 'DOM 自动阅卷', color: '#6366F1' },
    ...
    function handleDOMGrading() {
        ...
        try {
            ...
            const standardSelectors = [
                '.reference-answer', '.standard-answer', '.ans-box',
                '[class*="answer-content"]', '.correct-answer'
            ];
            let standardText = "";
            for (const sel of standardSelectors) {
                const el = document.querySelector(sel);
                if (el && el.innerText.trim()) {
                    standardText = extractCleanText(el);
                    console.log('✅ 提取到标准答案，来源:', sel);
                    break;
                }
            }
            ...
            const studentSelectors = [
                '.break-all.break-words', '.answer-box', '.markdown-latex-container',
                '.evaluation-content', '[class*="student-work"]', '.student-answer'
            ];
            let studentText = "";
            for (const sel of studentSelectors) {
                const el = document.querySelector(sel);
                if (el && el.innerText.trim()) {
                    studentText = extractCleanText(el);
                    console.log('✅ 提取到学生答案，来源:', sel);
                    break;
                }
            }

            if (!studentText) {
                updatePanelBody('<div style="color:#ef4444;text-align:center;padding:20px;">❌ 未检测到学生答案，请确保已进入批改页面</div>');
                animateRingStop();
                return;
            }
            ...
            updatePanelBody(`
                <div style="text-align:center; padding:20px;">
                    <div style="margin-bottom:15px;">🤖 AI 正在阅卷中...</div>
                    <div style="font-size:12px; color:#666; text-align:left; background:#f5f5f5; padding:10px; border-radius:8px;">
                        <strong>提取到的答案：</strong><br>
                        ${studentText.substring(0, 200)}${studentText.length > 200 ? '...' : ''}
                    </div>
                </div>
            `);
            ...
            chrome.runtime.sendMessage({
                ...
                homeworkData: {
                    question: standardText ? "参考标准答案：" + standardText : "请直接根据学生作答内容进行评价",
                    answer: studentText
                }
            }, (res) => {
                if (res && res.success) {
            ...
            console.error('❌ DOM 阅卷提取出错:', error);
            updatePanelBody(`<div style="color:red;padding:20px;">❌ 提取出错: ${error.message}</div>`);
            animateRingStop();
        }
    }
    ...
    function extractCleanText(element) {
        ...
        // 移除无关元素
        const junk = clone.querySelectorAll('script, style, button, input, .ignore-ai');
    ...
    function displayDOMGradingResult(data) {
        ...
        const feedback = data.totalFeedback || data.comment || "无评语";

        const html = `
            <div style="padding:10px;">
                <div style="background:#f0f7ff; border-radius:12px; padding:15px; margin-bottom:15px; border-left:4px solid #6366F1;">
                    <div style="font-size:24px; font-weight:bold; color:#6366F1; margin-bottom:5px;">${score} <span style="font-size:14px; color:#666; font-weight:normal;">/ 100分</span></div>
                    <div style="font-size:14px; color:#374151; white-space:pre-wrap;">${feedback}</div>
                </div>
        ...
        fillBtn.onclick = (e) => {
            e.stopPropagation();
            const success = autoFillGradeAndComment(score, feedback);
            if (success) {
                fillBtn.textContent = '✅ 填充完成';
                fillBtn.style.background = '#10b981';
                fillBtn.disabled = true;
            } else {
                alert('未找到填分输入框，请确保在批改界面');
            }
        };
    }
    ...
    function autoFillGradeAndComment(score, comment) {
        ...
        for (const sel of commentTextareas) {
            const el = document.querySelector(sel);
            if (el && el.offsetParent !== null) {
                el.value = comment;
                el.dispatchEvent(new Event('input', { bubbles: true }));
