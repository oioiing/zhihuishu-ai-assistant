...
(function() {
        ...
        const html = `
            <div style="max-height:500px; overflow-y:auto; padding:10px;">
                ...
                <div style="margin-top:16px; text-align:center; display:flex; gap:8px; justify-content:center;">
                    <button onclick="window.zhBackToDetection()" style="background:#4CAF50; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer; font-size:12px;">
                        🎯 重新选择
                    </button>
        ...
        let html = `
            <div style="padding:16px;">
                <div style="margin-bottom:16px; padding:12px; background:#e3f2fd; border-radius:8px; border-left:4px solid #2196F3;">
                    ...
                    <p style="margin:0; font-size:13px; color:#555;">请选择阅卷方式，或使用手动截图</p>
                </div>
            ...
            html += `
                ...
                ">
                    ...
                    <div style="display:flex; gap:8px;">
                        ...
                        " onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">🚀 DOM 阅卷</button>
                        <button class="zh-screenshot-grade-btn" data-index="${index}" style="
                            ...
                            font-weight:600;
                            transition:opacity 0.2s;
                        " onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">📸 截图阅卷</button>
                    </div>
                </div>
            `;
        });
    ...
    function extractHomeworkDataFromDOM(element) {
        ...
        // 移除不需要的元素
        const toRemove = clone.querySelectorAll('script, style, button, input[type="button"], input[type="submit"], .zh-ignore');
        ...
        console.log('✅ DOM 数据提取完成，长度:', text.length);
        return text;
    }

    function handleDOMGrading(area) {
        console.log('🚀 开始 DOM 自动阅卷...');
        animateRingStart('#FF6B6B');

        const text = extractHomeworkDataFromDOM(area.element);

        if (!text || text.length < 5) {
            updatePanelBody(`
                <div style="text-align:center; padding:30px; color:#666;">
                    <div style="font-size:48px; margin-bottom:16px;">⚠️</div>
                    <h3 style="margin:0 0 12px 0; color:#333;">提取内容不足</h3>
                    <p style="font-size:14px; line-height:1.6; margin-bottom:20px;">
                        无法从该区域提取到足够的文字内容。<br>
                        这可能是因为该区域主要是图片或复杂的交互组件。
                    </p>
                    <button onclick="window.zhBackToDetection()" style="background:#2196F3; color:white; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-size:14px; margin-right:8px;">
                        返回重选
                    </button>
                    <button onclick="window.zhScreenshotModeForArea(${window.detectedHomeworkAreas.indexOf(area)})" style="background:#FF6B6B; color:white; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-size:14px;">
                        📸 截图阅卷
                    </button>
                </div>
            `);
        ...
        updatePanelBody(`
            <div style="text-align:center; padding:30px;">
                ...
                <div style="font-size:12px; color:#666; background:#f5f5f5; padding:10px; border-radius:4px; text-align:left; max-height:150px; overflow-y:auto; border:1px solid #ddd; margin-top:15px;">
                    <div style="font-weight:bold; margin-bottom:5px; color:#FF6B6B;">提取到的文本：</div>
                    ${text}
                </div>
                <div style="margin-top:20px; font-size:13px; color:#888;">正在调用 DeepSeek AI 进行深度分析...</div>
            </div>
        `);
        ...
        chrome.runtime.sendMessage({
            ...
            if (response && response.success) {
                displayHomeworkAnalysis(response.data, null);
            } else {
                updatePanelBody(`
                    <div style="color:red; padding:20px; text-align:center;">
                        <div style="font-size:40px; margin-bottom:10px;">❌</div>
                        <div style="font-weight:bold; margin-bottom:10px;">阅卷失败</div>
                        <div style="font-size:13px;">${response ? response.error : '未知错误'}</div>
                        <button onclick="window.zhBackToDetection()" style="margin-top:20px; background:#FF6B6B; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">
                            返回重试
                        </button>
                    </div>
                `);
        ...
        // 添加辅助函数
        window.zhScreenshotModeForArea = (index) => {
            highlightAndCaptureArea(window.detectedHomeworkAreas[index]);
        };
    }
    ...
    console.log('  - testDOMGrading() - 测试 DOM 自动阅卷功能');
    console.log('  - fullFunctionTest() - 完整功能测试');

    window.testDOMGrading = function() {
        console.log('🧪 测试 DOM 自动阅卷功能...');
        const areas = detectHomeworkAreas();
        if (areas.length > 0) {
            console.log('✅ 检测到作业区域，开始提取测试...');
            const text = extractHomeworkDataFromDOM(areas[0].element);
            console.log('📄 提取到的文本预览:', text.substring(0, 100));
            handleDOMGrading(areas[0]);
        } else {
            console.log('❌ 未检测到作业区域，请在包含作业内容的页面运行');
        }
    };
