...
async function getApiKey() {
    const result = await chrome.storage.local.get(['zhai_api_key']);
    const key = result.zhai_api_key || DEFAULT_API_KEY;

    const placeholders = ['YOUR_API_KEY_HERE', 'YOUR_NEW_API_KEY_HERE', 'YOUR_REAL_API_KEY_HERE'];
    if (placeholders.includes(key)) {
        throw new Error('❌ API 密钥未配置，请在插件设置中设置。');
    }
    return key;
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            gradeHomeworkWithAI(request.homeworkData)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
async function performHomeworkGrading(homeworkText) {
    ...
    try {
        const deepseekApiKey = await getApiKey();
        const response = await fetch(API_URL, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function gradeHomeworkWithAI(homeworkData) {
    const API_KEY = await getApiKey();
    const API_URL = "https://api.deepseek.com/chat/completions";

    const prompt = `
你是一个专业的助教。请对以下作业进行阅卷。

【题目】
${homeworkData.question || '见学生答案'}

【标准答案】
${homeworkData.standardAnswer || '未提供'}
...
{
    "score": 85,
    "feedback": "这里写一段通俗易懂的评语，指出优点和不足（约100字）。"
}
`;
    ...
    try {
        ...
        throw new Error("AI 阅卷失败，请检查网络或 API Key。");
    }
}
