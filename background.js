...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomework':
            analyzeHomeworkWithAI(request.imageData, request.selectionInfo, request.domContent)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
// 智慧作业分析函数 (支持 DOM 内容和图像 OCR)
async function analyzeHomeworkWithAI(imageData, selectionInfo, domContent) {
    try {
        console.log('=== 开始作业分析流程 ===');

        if (domContent && domContent.trim().length > 0) {
            console.log('✅ 发现 DOM 内容，直接进行 AI 分析...');
            const analysisResult = await analyzeTextContent(domContent);
            return analysisResult;
        }

        console.log('ℹ️ 未提供 DOM 内容或内容为空，切换到图像分析模式...');
        if (!imageData) {
            throw new Error('缺少可分析的内容（既无 DOM 内容也无图像数据）');
        }
...
async function callDeepSeekAPI(message) {
    ...
    // 检查API Key
    if (!API_KEY || API_KEY === "YOUR_NEW_API_KEY_HERE" || API_KEY === "YOUR_REAL_API_KEY_HERE" || API_KEY === "YOUR_API_KEY_HERE") {
