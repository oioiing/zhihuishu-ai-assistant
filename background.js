...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            analyzeHomeworkTextWithAI(request.studentAnswer, request.standardAnswer, request.requirements)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
// 文本作业分析函数
async function analyzeHomeworkTextWithAI(studentAnswer, standardAnswer, requirements) {
    try {
        console.log('=== 开始文本作业分析流程 ===');
        const analysisResult = await analyzeTextContent(`
【作业要求/题目】
${requirements || '未提供题目信息'}

【标准答案】
${standardAnswer || '未提供标准答案'}
