...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            (async () => {
                try {
                    console.log('📝 收到文本阅卷分析请求');
                    const { homeworkData } = request;
                    const result = await analyzeHomeworkTextDirect(homeworkData);
                    sendResponse({ success: true, data: result });
                } catch (error) {
                    console.error('❌ 文本阅卷分析失败:', error);
                    sendResponse({ success: false, error: error.message });
                }
            })();
            return true;
...
async function analyzeHomeworkTextDirect(homeworkData) {
    try {
        console.log('🎯 开始文本作业阅卷流程...');

        let homeworkText = '';
        if (homeworkData.question) {
            homeworkText += `【作业题目】\n${homeworkData.question}\n\n`;
        }
        if (homeworkData.standardAnswer) {
            homeworkText += `【标准答案】\n${homeworkData.standardAnswer}\n\n`;
        }
        ...
        console.log('🤖 准备调用AI评分...');
        const gradingResult = await performHomeworkGrading(homeworkText);
        console.log('✅ AI评分分析完成');

        return gradingResult;

    } catch (error) {
        console.error('❌ 文本作业阅卷分析失败:', error);
        throw error;
    }
}
...
async function performHomeworkGrading(homeworkText) {
    ...
    const gradingPrompt = `你是一位有经验的教师，需要对学生的作业进行批改。
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    { role: "system", content: "你是一位专业的教师，精通作业批改和成绩评定。必须严格返回JSON格式。" },
