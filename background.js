...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            gradeHomeworkWithAI(request.homeworkData.question, request.homeworkData.answer)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
// ==========================================
// 3. 自动阅卷 (直接处理 DOM 提取的文本)
// ==========================================
async function gradeHomeworkWithAI(standard, student) {
    const API_KEY = "YOUR_API_KEY_HERE";
    const API_URL = "https://api.deepseek.com/chat/completions";

    const prompt = `
你是一个专业的教师。请根据标准答案（如果有）对学生的作答进行阅卷评分。
...
{
    "totalScore": 85,
    "totalFeedback": "这里写一段针对学生作答的详细评语，指出优点和不足。100字左右。"
}
`;
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    { role: "system", content: "你是一个专业的自动阅卷助手，请根据提供的标准答案和学生作答给出评分和建议。仅输出 JSON。" },
        ...
        return {
            totalScore: 0,
            totalFeedback: `阅卷失败：${error.message}。请检查 API Key 或网络。`
        };
    }
}