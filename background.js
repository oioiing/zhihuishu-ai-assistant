...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            analyzeHomeworkTextWithAI(request.homeworkData)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
                    console.error('analyzeHomeworkText 失败:', error);
                    sendResponse({ success: false, error: error.message });
                });
            return true;
...
// 分析文字内容（使用自定义批改规则）
async function analyzeHomeworkTextWithAI(homeworkData) {
    const API_KEY = "YOUR_API_KEY_HERE";  // 用户的真实API密钥
    const API_URL = "https://api.deepseek.com/chat/completions";

    try {
        const { standardAnswer, studentAnswer } = homeworkData;
        console.log('开始AI阅卷，标准答案长度:', standardAnswer?.length, '学生答案长度:', studentAnswer?.length);

        const prompt = `
你是一个专业的助教，请根据以下参考答案和学生答案进行批改。
...
1. 给出 0-100 的评分（数字）。
2. 给出详细的评语和改进建议。
3. 如果是选择题，请严格核对字母。如果是问答题，请根据核心考点匹配度进行评分。
...
`;
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    { role: "system", content: "你是一个善于自动阅卷的AI助教，请以JSON格式返回结果。" },
