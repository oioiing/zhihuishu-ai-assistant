...
async function getApiKey() {
    return new Promise((resolve) => {
        chrome.storage.local.get(['zhai_api_key'], (result) => {
            const key = result.zhai_api_key || "YOUR_API_KEY_HERE";
            resolve(key);
        });
    });
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            analyzeHomeworkTextWithAI(request.data)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
// 分析文字内容（使用自定义批改规则）
async function analyzeHomeworkTextWithAI(data) {
    const API_KEY = await getApiKey();
    const API_URL = "https://api.deepseek.com/chat/completions";

    try {
        console.log('开始 AI 阅卷分析...');

        const prompt = `
请作为专业的教师，根据以下内容对学生的作业进行阅卷评分。

【标准答案/题目内容】
${data.standardAnswer || '未提供'}

【学生作答内容】
${data.studentAnswer || '未提供'}

请严格按照以下 JSON 格式返回结果（不要包含任何 markdown 标记）：
{
  "score": 建议分数(0-100的数字),
  "feedback": "详细的批改评语，指出优点和不足，并给出改进建议"
}
`;
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    { role: "system", content: "你是一位专业的助教，擅长根据标准答案对学生作业进行评分和提供反馈。" },
        ...
        try {
            ...
            return {
                ...
                feedback: feedbackMatch ? feedbackMatch[1] : content
            };
        }
...
async function analyzeTextContent(text) {
    const API_KEY = await getApiKey();  // 用户的真实API密钥
    const API_URL = "https://api.deepseek.com/chat/completions";
...
async function callDeepSeekAPI(message) {
    ...
    // 检查API Key
    if (!API_KEY || API_KEY === "YOUR_API_KEY_HERE" || API_KEY === "YOUR_NEW_API_KEY_HERE" || API_KEY === "YOUR_REAL_API_KEY_HERE") {
        throw new Error("❌ API密钥未配置\n\n请按以下步骤配置：\n1. 访问 https://platform.deepseek.com\n2. 注册并获取API密钥\n3. 在插件设置中配置 zhai_api_key\n4. 确保账户有足够余额");
    }
