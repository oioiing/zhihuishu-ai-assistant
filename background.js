...
const DEFAULT_API_KEY = "sk-6f2c1a0e4f6c4274a3abd1754777655b";
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        ...
        case 'analyzeHomeworkText':
            gradeHomeworkWithAI(request.homeworkText)
                .then(response => {
                    sendResponse({ success: true, data: response });
                })
                .catch(error => {
...
// AI 智能阅卷函数 (DOM 文本版)
async function gradeHomeworkWithAI(homeworkText) {
    const API_KEY = await getApiKey();
    const API_URL = "https://api.deepseek.com/chat/completions";

    try {
        console.log('开始 AI 智能阅卷 (文本版)，长度:', homeworkText.length);

        const prompt = `
你是一位专业的教师。请根据以下作业内容进行批改和评分。
...
{
    "totalScore": 85,
    "totalFeedback": "这里写详细的批改建议、优点、改进点及学习建议。"
}
`;
        ...
        const requestBody = {
            ...
            messages: [
                {
                    role: "system",
                    content: "你是一位经验丰富的教师，擅长批改各类作业。请提供专业、详细、有建设性的批改建议，使用中文回复。必须返回 JSON 格式。"
                },
                {
                    role: "user",
                    content: prompt
