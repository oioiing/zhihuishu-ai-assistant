...
async function analyzeHomeworkTextDirect(homeworkData) {
    ...
    try {
        ...
        console.log('📊 文本长度:', homeworkText.length);
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: "deepseek-chat",
                messages: [
                    { role: "system", content: "你是一位专业的教师，精通批改和成绩评定。必须返回有效的JSON格式。" },
...
function parseGradingResult(text) {
    ...
    const totalMatch = text.match(/(?:总分|得分)[：:]*\s*(\d+)/);
    if (totalMatch) {
